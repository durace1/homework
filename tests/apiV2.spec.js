...
test.describe('GET', () => {
    ...
    test('#05 GET /todos/id(200)',{
    tag: ['@GET'],}, async ({ api }) => {
    const id = 10;
    const response = await api.todos.getId(token, id);
    await expect (response.status()).toBe(200);
    const body = await response.json();
    expect (body.todos[0].title).toBe('install webcam');
    });
    
    test('#06 GET /todos/id(404)',{
    tag: ['@GET'],}, async ({ api }) => {
    const id = 100;
    const response = await api.todos.getId(token, id);
    await expect (response.status()).toBe(404);
...
test.describe('POST Challenges', () => {
    ...
    test('#09POST /todos(400)',{
    tag: ['@POST'],}, async ({ api }) => {
    const wrongData = 123
    const randomCreate = new builder().createTitle().createDescription().createStatus().generate();
    const response = await api.todos.post(token, randomCreate.title, randomCreate.description, wrongData);
    const body = await response.json();
    await expect (response.status()).toBe(400);
    await expect (body.errorMessages).toEqual(['Failed Validation: doneStatus should be BOOLEAN but was NUMERIC']);
    })

    test('#10POST /todos(400)',{
    tag: ['@POST'],}, async ({ api }) => {
    const wrongData = 'The quick brown fox jumps over the lazy dog repeatedly, demonstrating agility and speed while the sun shines brightly in the clear blue sky on a warm summer afternoon.'
    const randomCreate = new builder().createTitle().createDescription().createStatus().generate();
    ...
    test('#11POST /todos(400)',{
    tag: ['@POST'],}, async ({ api }) => {
    const wrongData = 'The quick brown fox jumps over the lazy dog repeatedly, demonstrating agility and speed while the sun shines brightly in the clear blue sky on a warm summer afternoon.The quick brown fox jumps over the lazy dog repeatedly, demonstrating agility and speed while the sun shines brightly in the clear blue sky on a warm summer afternoon.'
    const randomCreate = new builder().createTitle().createDescription().createStatus().generate();
    ...
    test('#12POST /todos(201)',{
    tag: ['@POST'],}, async ({ api }) => {
    const title = 'The sun shines brightly as birds sing singsingsing';
    const description = 'The sun shines brightly as birds sing singsingsingThe sun shines brightly as birds sing singsingsingThe sun shines brightly as birds sing singsingsingThe sun shines brightly as birds sing singsingsing'
    const randomCreate = new builder().createTitle().createDescription().createStatus().generate();
    ...
    test('#14POST /todos(400)',{
    tag: ['@POST'],}, async ({ api }) => {
    const priority = 'extra';
    const randomCreate = new builder().createTitle().createDescription().createStatus().generate();
    const response = await api.todos.postFake(token, randomCreate.title, randomCreate.description, randomCreate.status, priority);
    expect (response.status()).toBe(400);
    })

    test('#17POST /todos(201)',{
    tag: ['@POST'],}, async ({ api }) => {
    const id = 6;
    const randomCreate = new builder().createTitle().createDescription().createStatus().generate();
    const response = await api.todos.postId(token, id, randomCreate.title, randomCreate.description, randomCreate.status);
    expect (response.status()).toBe(200);
    const body = await response.json();
    expect (body.title).toBe(randomCreate.title);
    })

    test('#18POST /todos(404)',{
    tag: ['@POST'],}, async ({ api }) => {
    const id = 99;
    const randomCreate = new builder().createTitle().createDescription().createStatus().generate();
    const response = await api.todos.postId(token, id, randomCreate.title, randomCreate.description, randomCreate.status);
...
test.describe('PUT Challenges', () => {
    ...
    test('#20PUT /todos(200)',{
    tag: ['@PUT'],}, async ({ api }) => {
    const id = 1;
    const randomCreate = new builder().createTitle().createDescription().createStatus().generate();
    const response = await api.todos.putTitle(token, id, randomCreate.title);
    expect (response.status()).toBe(200);
    const body = await response.json();
    expect (body.title).toBe(randomCreate.title);
    })

    test('#21PUT /todos(400)',{
    tag: ['@PUT'],}, async ({ api }) => {
    const id = 1;
    const randomCreate = new builder().createTitle().createDescription().createStatus().generate();
    const response = await api.todos.putNoTitle(token, id, randomCreate.description, randomCreate.status);
    expect (response.status()).toBe(400);
    expect (response.statusText()).toBe('Bad Request');
    })

    test('#22PUT /todos(400)',{
    tag: ['@PUT'],}, async ({ api }) => {
    const id = 6;
    const newId = 12;
    const randomCreate = new builder().createTitle().createDescription().createStatus().generate();
    const response = await api.todos.putNewTitle(token, id, newId, randomCreate.title, randomCreate.description, randomCreate.status);
    expect (response.status()).toBe(400);
    expect (response.statusText()).toBe('Bad Request');
    })
    })
...
test.describe('DELETE Challenges', () => {
    ...
    test('#99 GET /todos?filter(200)',{
        tag: ['@GET'],}, async ({ api }) => {
        const filter = '?doneStatus=true';
        const response = await api.todos.get(token, filter);
        await expect (response.status()).toBe(200);
        const body = await response.json();
        await expect (body.todos.length).toBe(1);
        });
    })